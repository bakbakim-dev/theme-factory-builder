...
const themeDownloadTarget = path.join(repoRoot, 'logs', 'whipify-forms-theme-download.zip');
const pluginDownloadTarget = path.join(repoRoot, 'logs', 'whipify-forms-plugin-download.zip');
...
test('wordpress mode exports a Whipify forms manifest and bundled plugin', async ({ page }) => {
  ...
  await fs.mkdir(path.dirname(themeDownloadTarget), { recursive: true });
  ...
  await fs.mkdir(path.dirname(pluginDownloadTarget), { recursive: true });
