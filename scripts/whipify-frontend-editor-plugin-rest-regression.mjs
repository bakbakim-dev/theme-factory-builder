...
assert.match(frontendEditorPluginPhp, /tf_frontend_editor_apply_page_block_adapter_update/);
...
assert.match(artifacts.php, /transport' => function_exists\( 'tfb_frontend_editor_rest_base_url' \) \? 'rest' : 'ajax'/);
...
assert.match(artifacts.js, /saveGlobalChromeViaAjax/);
assert.match(artifacts.js, /savePageBlockViaAjax/);
assert.match(artifacts.js, /lockCurrentPagePost/);
assert.match(artifacts.js, /whipifyFrontendEditorConfig\.transport === 'rest'/);
