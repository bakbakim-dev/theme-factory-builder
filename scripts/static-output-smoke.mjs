...
const readArtifactInput = async () => {
  ...
  if (!artifactRoot) {
    ...
    const routes = fixtureSlugs.map((slug) => {
      const routePath = slug === 'home' ? '/' : `/${slug}/`;
      const title = slug === 'home' ? 'Home' : slugToTitle(slug);
      ...
      routeHtmlByPath[routePath] = `<!DOCTYPE html><html><head><title>${title}</title></head><body>${body}</body></html>`;
      return { path: routePath, slug, title };
    });

    return { routes, routeHtmlByPath, source: 'fixture' };
  }
  ...
  return { routes, routeHtmlByPath, source: 'artifact' };
};

const { routes, routeHtmlByPath, source } = await readArtifactInput();
...
console.log(`[PASS] Static export generated for ${routes.length} routes (${source})`);
