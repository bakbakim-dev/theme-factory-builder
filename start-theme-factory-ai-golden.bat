...
echo [Theme Factory] Writing local build URL to .env.local...

(
  echo VITE_BUILD_SERVER_URL=http://localhost:%BUILDER_PORT%/build
) > "%ENV_FILE%"
...
if errorlevel 1 (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$c = Get-NetTCPConnection -LocalPort %BUILDER_PORT% -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1; if ($c -and $c.OwningProcess) { Stop-Process -Id $c.OwningProcess -Force -ErrorAction SilentlyContinue }; [Environment]::SetEnvironmentVariable('PORT','%BUILDER_PORT%','Process'); [Environment]::SetEnvironmentVariable('THEME_FACTORY_APP_DIR','%APP_DIR%','Process'); Start-Process -FilePath '%NODE_EXE%' -ArgumentList 'index.js' -WorkingDirectory '%BUILDER_DIR%' -WindowStyle Hidden"
)
...
if errorlevel 1 (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$c = Get-NetTCPConnection -LocalPort %UI_PORT% -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1; if ($c -and $c.OwningProcess) { Stop-Process -Id $c.OwningProcess -Force -ErrorAction SilentlyContinue }; [Environment]::SetEnvironmentVariable('PORT','%UI_PORT%','Process'); Start-Process -FilePath '%NPM_CMD%' -ArgumentList 'run','dev','--','--port','%UI_PORT%','--host','127.0.0.1' -WorkingDirectory '%APP_DIR%' -WindowStyle Hidden"
)
